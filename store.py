...
def _migration_002(cur):
    """The append-only event log, and each output's cursor into it.

    This is the seam between capture and presentation. Everything the system
    observes lands here first; Discord, the web feed and any future transport
    are readers that track their own position. An output that is down falls
    behind instead of losing data, which is the failure the old inline sends
    could not express — they swallowed the exception and moved on.

    `seq` is the cursor: a plain AUTOINCREMENT integer, monotonic within this
    host and never reused. The envelope's ULID `id` is the identity that
    survives leaving the host, and it is UNIQUE so replaying a batch is a
    ...
    """)
    cur.execute("CREATE INDEX events_created_at ON events(created_at)")
    cur.execute("CREATE INDEX events_type ON events(type)")
    cur.execute("""
...
# ---------------------------------------------------------------------------
class EventLog:
    """Append-only log of everything the system has observed.

    Producers append and never block on a consumer; consumers read forward from
    a cursor they own. See docs/architecture.md.
    """
    ...
    def append(self, event, now=None):
        """Append an envelope. Returns its seq.

        Idempotent on the envelope's ULID: appending the same event twice
        returns the original seq without writing again, so a retried batch or a
        ...
        with self.store.lock:
            ...
            cur.execute(
                ...
                "(id, ts, created_at, type, source, group_name, stream, tier, body) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (event["id"], event["ts"], now, event["type"], event.get("source"),
                 event.get("group"), event.get("stream"), event["tier"], body))
            if cur.rowcount == 0:
                seq = cur.execute("SELECT seq FROM events WHERE id = ?",
                                  (event["id"],)).fetchone()[0]
            else:
                seq = cur.lastrowid
            self.store.conn.commit()
    ...
    def count(self):
        with self.store.lock:
            return self.store.conn.execute("SELECT COUNT(*) FROM events").fetchone()[0]

    # -- cursors ------------------------------------------------------------
    def cursor(self, output):
        """Where `output` has read up to. 0 means it has consumed nothing."""
        with self.store.lock:
    ...
    def advance(self, output, seq, now=None):
        ...
        Never moves a cursor backwards. A consumer that crashes mid-batch
        re-reads from its last committed position, so delivery is at-least-once
        and the far side is expected to tolerate a repeat — which is what the
        envelope's stable ULID is for.
        """
        now = time.time() if now is None else now
        with self.store.lock:
            self.store.conn.execute(
                "INSERT INTO output_cursors (output, seq, updated_at) VALUES (?, ?, ?) "
    ...
    def cursors(self):
        with self.store.lock:
            rows = self.store.conn.execute(
                "SELECT output, seq FROM output_cursors").fetchall()
        return {row["output"]: row["seq"] for row in rows}

    # -- maintenance --------------------------------------------------------
    def sweep(self, cutoff):
        """Drop events older than `cutoff` (epoch seconds). Returns
        (removed, unconsumed) where `unconsumed` counts events dropped that a
        registered output had not reached yet.

        Retention is a hard bound, so a stuck output does not get to grow the
        database without limit — but silently discarding its backlog is how you
        find out about it six weeks later. The caller reports the count.
        """
        with self.store.lock:
            cur = self.store.conn.cursor()
            cursors = [r[0] for r in
                       cur.execute("SELECT seq FROM output_cursors").fetchall()]
            unconsumed = 0
            if cursors:
                unconsumed = cur.execute(
                    "SELECT COUNT(*) FROM events WHERE created_at < ? AND seq > ?",
                    (cutoff, min(cursors))).fetchone()[0]
            cur.execute("DELETE FROM events WHERE created_at < ?", (cutoff,))
            removed = cur.rowcount
            self.store.conn.commit()
        return removed, unconsumed
...
class Store:
    ...
    def reconciler(self, table, key, tracked=(), scope=""):
        ...
        Two watchers sharing a scope is the one misconfiguration that fails
        quietly and forever — each reads the other's rows as missing from its
        own snapshot, so the pair flaps between cleared and re-declared on every
        poll. The store cannot detect it, because what makes two watchers
        different is their upstream filter and the store never sees that. The
        check lives with the caller that knows: see check_scopes() in
        scanner_transcribe_gpu.py.
        """
        return Reconciler(self, table, key, tracked, scope)

    # -- maintenance --------------------------------------------------------
    def sweep(self, now=None):
        """Drop rows and events older than the retention window.
        ...
        Returns (rows_removed, events_removed, events_unconsumed). A non-zero
        third value means an output is more than the retention window behind
        and its backlog was dropped; the caller is expected to say so loudly.
        """
        now = time.time() if now is None else now
        ...
        with self.lock:
            ...
            for table in RECONCILED_TABLES:
                ...
                removed += cur.rowcount
            self.conn.commit()
        events_removed, unconsumed = self.events.sweep(cutoff)
        return removed, events_removed, unconsumed
...
def main():
    ...
    if args.sweep:
        ...
        print(f"[store] Swept {removed} row(s) and {events_removed} event(s) "
              f"older than {args.retention_days}d")
        if unconsumed:
            print(f"[store] WARNING: {unconsumed} dropped event(s) had not been "
                  f"consumed by every output: {store.events.cursors()}")
        return
